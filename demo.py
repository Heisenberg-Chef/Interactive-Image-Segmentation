...
def apply_mask(image, mask, filename,alpha_channel=True):  # 应用并且响应mask
    if alpha_channel:
        alpha = np.zeros_like(image[..., 0])  # 制作掩体
        alpha[mask == 1] = 255  # 兴趣地方标记为1，且为白色
        cv2.imwrite("./mask/"+ filename + ".png",alpha)
        image = cv2.merge((image[..., 0], image[..., 1], image[..., 2], alpha))  # 融合图像
    else:
        image = np.where(mask[..., None] == 1, image, 0)
    return image
        ...
        if key == ord(" "):
            ...
            if len(input_point) > 0 and len(input_label) > 0:
                ...
                while (1):
                    ...
                    save_mask = np.where(selected_mask == True,255,0)
                    cv2.imwrite("./mask/" + filename[:filename.rfind(".")] + ".png", save_mask)
